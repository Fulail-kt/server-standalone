...
// type IRange = 'today'|'tomorrow'|'all' 
...
interface FoodDocument {
  ...
  date: string;
  mealTimes: string[];
  sponsorId: { toString(): string } | string;
}

interface PopulatedFoodDocument extends Omit<FoodDocument, 'sponsorId'> {
  sponsorId: {
    _id: string | { toString(): string };
    name?: string;
  };
...
export const foodRouter = router({
  ...
  getUserBookings: privateProcedure.query(async ({ ctx }) => {
    const sponsorships = await FoodModel.find({ sponsorId: ctx.user.userId }).lean();
    return sponsorships.map((s: FoodDocument) => ({
      id: s._id.toString(),
      date: s.date,
      mealTimes: s.mealTimes,
      sponsorId: typeof s.sponsorId === 'string' ? s.sponsorId : s.sponsorId.toString(),
    }));
  }),
  ...
  create: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await FoodModel.find({ date: input.date });
      const bookedMeals = existing.flatMap((s: FoodDocument) => s.mealTimes);
